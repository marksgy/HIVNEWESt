function tansferCoords(angle, radius) {
  ...
  return { x: Math.sin(radian) * radius, y: Math.cos(radian) * radius };
}
...
function radar(width, height, data, labels, series, colours, options, successFn) {
  var center = { x: width / 2, y: height / 2 };// 雷达圆心
  ...
  ctx.setStrokeStyle("rgba(0, 0, 0, .1)");// 底色暂时固定
  /* 绘制半径 */
  ...
  for (var i in labels) {
    ...
    var offset = { x: -1 * labels[i].length / 2 * 14 };
    var rotAgl = null;
    var above = null;
